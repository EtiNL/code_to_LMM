...
def aggregate_code_from_folder(folder_path):
    ...
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            ...
            if (file.endswith('.json') or file.endswith('.rs') or file.endswith('.py') or file.endswith('.cu') or file.endswith('Dockerfile') or file.endswith('.md')) and 'target' not in root.split(os.sep):
